...
/*
  ...
  private class TestTransformer implements Transformer<String, String, KeyValue<String, String>> {
    ...
    public void init(ProcessorContext context) {
      this.store = (GigaStateStore) context.getStateStore(GigaStateStore.STORE_NAME);
    }
  ...
  private TopologyTestDriver buildTopologyDriver(TransformerSupplier<String, String, KeyValue<String, String>> transformerSupplier) {
    ...
    builder.addStateStore(new GigaStoreBuilder(container.getHttpHostAddress()));

    builder
        .stream(SOURCE_TOPIC, Consumed.with(Serdes.String(), Serdes.String()))
        .transform(transformerSupplier, GigaStateStore.STORE_NAME)
        .to(TARGET_TOPIC);
  ...
  public void testStoreSetup() {
    ...
    GigaStateStore store = (GigaStateStore)driver.getStateStore(GigaStateStore.STORE_NAME);
